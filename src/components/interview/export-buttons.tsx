...
function generateMarkdown(report: InterviewReport, session: InterviewSession): string {
  ...
  lines.push(`## Round-by-Round Evaluation\n`);
  for (const r of rounds) {
    ...
    for (const q of r.questions) {
      ...
      lines.push(`**Answer Summary**: ${q.answerSummary}\n`);
      if (q.highlights.length) lines.push(`✅ **Highlights**: ${q.highlights.join('；')}\n`);
      if (q.weaknesses.length) lines.push(`❌ **Areas for improvement**: ${q.weaknesses.join('；')}\n`);
  ...
  for (const item of report.improvementPlan) {
    const label = { high: '🔴 High', medium: '🟡 Medium', low: '🟢 Low' }[item.priority] || item.priority;
    lines.push(`### [${label}] ${item.area}\n`);
    lines.push(`${item.description}\n`);
    if (item.resources.length) lines.push(`> Recommended resources: ${item.resources.join('；')}\n`);
  }

  lines.push(`\n---\n*Generated by NexoraAI*`);
  return lines.join('\n');
}
