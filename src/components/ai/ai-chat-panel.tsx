...
export function AIChatContent({ resumeId, hideTitle }: AIChatContentProps) {
  ...
  useEffect(() => {
    if (chatError && chatError !== lastErrorRef.current) {
      ...
      if (msg.includes('ETIMEDOUT') || msg.includes('Cannot connect')) {
        toast.error(t('errorMessage'), { description: 'AI API timed out. Check your network or API settings.' });
      } else if (msg.includes('No tool call found')) {
        toast.error(t('errorMessage'), { description: 'The AI model returned an invalid tool call. Please try again.' });
      } else {
  ...
  const handleSubmit = useCallback((e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (activeSession && activeSession.title === 'New Chat' && input.trim()) {
